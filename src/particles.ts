...
Transform.create(particleParentEntity, {
  position: Vector3.create(96, 0, 80),
  rotation: Quaternion.fromEulerDegrees(0, 90, 0),
  scale: Vector3.create(4,1,6)
})

// Position particles 2
export const particleParentEntity2 = engine.addEntity()
Transform.create(particleParentEntity2, {
...
for (let i = 0; i < MAX_PARTICLES; i++) {
  ...
  const particleEntity2 = engine.addEntity()
  MeshRenderer.setPlane(particleEntity2)
  Material.setPbrMaterial(particleEntity2, material)
  Particle.create(particleEntity2, {
    life: Math.random(),
    seed: 16 * Math.random(),
    height: 7,
    speed: 0.25
  ...
  Transform.create(particleEntity2, {
    rotation: Quaternion.fromEulerDegrees(0, 90, 0),
    scale: Vector3.create(0.01, 0.1, 1),
    parent: particleParentEntity2
  })
}
